...
export function ImportSubCat({ subCategories, onSuccess }: any) {
    ...
    async function handleImport() {
        if (!file) return;
        setImporting(true);
        const tid = showLoading("Processing...");
        router.post(route("subCat.import"), { file }, {
            forceFormData: true,
            onSuccess: () => { hideLoading(tid); setOpen(false); if (onSuccess) onSuccess(); },
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader><DialogTitle>Import Sub-Categories</DialogTitle></DialogHeader>
                <div className="space-y-4">
                    <ImportValidation errors={serverErrors?.import || []} />
                    <Input type="file" accept=".csv" onChange={(e) => setFile(e.target.files?.[0] || null)} />
                    <Button onClick={handleImport} disabled={!file || importing} className="w-full">{importing ? "Processing..." : "Import"}</Button>
                </div>
            </DialogContent>
        </Dialog>
    )
