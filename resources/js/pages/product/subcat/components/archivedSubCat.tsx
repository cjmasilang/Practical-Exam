...
export function ArchivedSubCat() {
    ...
    const fetchArchived = async () => {
        setLoading(true);
        try {
            const res = await fetch(route('subCat.archived'), { headers: { 'Accept': 'application/json' } });
    ...
    const handleRestore = (id: number) => {
        ...
        router.post(route('subCat.restore', { id }), {}, { onSuccess: () => { hideLoading(tid); fetchArchived(); } });
    };

    const handlePermanentDelete = () => {
        if (!idToDelete) return;
        const tid = showLoading("Deleting...");
        router.delete(route('subCat.destroy', { id: idToDelete }), { onSuccess: () => { hideLoading(tid); setDeleteDialogOpen(false); fetchArchived(); } });
    };
        ...
        <>
            <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (v) fetchArchived(); }}>
                <DialogTrigger asChild>
                    <Button variant="outline" className="rounded-xl font-bold"><ArchiveRestore className="mr-2 h-4 w-4" /> Archive</Button>
                </DialogTrigger>
                <DialogContent className="max-w-2xl rounded-xl">
                    <DialogHeader><DialogTitle>Archived Sub-Categories</DialogTitle></DialogHeader>
                    ...
                    <div className="max-h-[400px] overflow-auto border rounded-xl">
                        {loading ? <div className="p-12 text-center"><Loader2 className="animate-spin mx-auto" /></div> : (
                            <table className="w-full text-sm">
                                <thead><tr className="border-b"><td className="p-3">Name</td><td className="p-3">Deleted</td><td className="p-3 text-right">Action</td></tr></thead>
                                <tbody>{archived.filter(i => i.name.toLowerCase().includes(search.toLowerCase())).map((item) => (
                                    <tr key={item.id} className="border-b">
                                        <td className="p-3">{item.name}</td>
                                        <td className="p-3">{format(new Date(item.deleted_at), 'MMM dd, yyyy')}</td>
            ...
            <ConfirmDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirm={handlePermanentDelete} title="Confirm Permanent Delete" />
        </>
    );
}
